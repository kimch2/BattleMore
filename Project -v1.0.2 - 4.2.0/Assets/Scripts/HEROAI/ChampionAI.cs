...
public class ChampionAI : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Time.time > nextUpdateTime || myManager.getState() is DefaultState)
        {
            ...
            currentTarget = AbilityAssessor.EvaluateAutoAttack(myManager, currentTarget, TargetAquisitionSkill);
            ...
            if (currentTarget)
            {
                if (Time.time > NextSpellCastTime)
                {
                    ...
                    if (Random.value < .3f)
                    {
                        ab = AbilityAssessor.GetDamageEnhancer(myManager, LastSpellCastTime);
                        if (ab)
                        {
                            StopAllCoroutines();
                            currentActionItem = StartCoroutine(CastSpell(this.gameObject.transform.position, gameObject, myManager.abilityList.IndexOf(ab), ReactionTime));
                    ...
                    else
                    {
                        if (Random.value < .5f)
                        {
                            ab = AbilityAssessor.GetSummonAbility(myManager, LastSpellCastTime);
                        }
                        else
                        {
                            ab = AbilityAssessor.GetDamageAbility(myManager, LastSpellCastTime);
                        }
    ...
    public void SeeCircleWarningZone(Vector3 point, float TimeTilStrike, float radius)
    {
        ...
        Ability BlockAb = AbilityAssessor.CanBlock(myManager);
